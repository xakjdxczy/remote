...
bool install_vcam(std::string* message) {
  ...
  const bool ok = register_dshow(kCamClsid, kVideoCat, L"尘埃X 摄像头", dll.c_str(),
                                 {0xa71c3e80, 0x6d42, 0x4f1b, {0x9e, 0x3a, 0xc4, 0xd8, 0xb2, 0xe9, 0x1f, 0x70}},
                                 {0x860BB310, 0x5D01, 0x11d0, {0xBD, 0x3B, 0x00, 0xA0, 0xC9, 0x11, 0xCE, 0x86}},
                                 MEDIATYPE_Video, MEDIASUBTYPE_RGB24);
  if (message) {
    *message = ok ? "已注册尘埃X 摄像头。OBS 选「视频采集设备 → 尘埃X 摄像头」。"
                  : "注册虚拟摄像头失败。";
  }
  return ok;
}
...
bool install_vmic(std::string* message) {
  ...
  const bool ok = register_dshow(kMicClsid, kAudioCat, L"尘埃X 麦克风", dll.c_str(),
  ...
  if (message) {
    *message = ok ? "已注册尘埃X 麦克风。OBS 视频采集设备里可把音频选成它。"
                  : "注册虚拟麦克风失败。";
  }
  return ok;
}
