(function (root) {
  ...
  function sharePassword(doc, id, password) {
    ...
    ["remote", "mesh"].forEach((kind) => {
      ...
      list.unshift({
        kind, id: pid, password: pw, pair_token: "", name: "", transport: "",
  ...
  function peerPassword(doc, id) {
    const pid = digits(id);
    if (!pid || !doc) return "";
    const mesh = (doc.mesh || []).find((x) => x.id === pid);
    const remote = (doc.remote || []).find((x) => x.id === pid);
    const a = recency(mesh);
    const b = recency(remote);
    if (mesh && mesh.password && a >= b) return mesh.password;
    return (remote && remote.password) || (mesh && mesh.password) || "";
  }

  function peerToken(doc, id) {
    const pid = digits(id);
    if (!pid || !doc) return "";
    const mesh = (doc.mesh || []).find((x) => x.id === pid);
    const remote = (doc.remote || []).find((x) => x.id === pid);
    return (mesh && mesh.pair_token) || (remote && remote.pair_token) || "";
  }

  function recency(it) {
    return Number(it && (it.last_at || it.ended_at || it.started_at) || 0) || 0;
  }
  ...
  async function setWant(kind, id, want) {
    return upsert(kind, { id, want: !!want });
  }
  ...
  function formatRange(started, ended) {
    const a = formatWhen(started);
    const b = formatWhen(ended);
  ...
  function formatLive(started) {
    const n = Number(started || 0);
    if (!n) return "";
    const ms = n > 1e12 ? n : n * 1000;
    return `开始 ${formatWhen(n)} · 已持续 ${formatDuration(Date.now() - ms)}`;
  }

  function formatRecord(it) {
    if (!it) return "";
    if (it.started_at && !it.ended_at) return formatLive(it.started_at);
    const span = formatRange(it.started_at, it.ended_at);
    if (span) return span;
    const last = formatWhen(it.last_at);
    return last ? `上次 ${last}` : "";
  }

  root.dustxRecents = {
    load, upsert, forget, setWant, formatWhen, formatDuration, formatRange, formatLive, formatRecord, digits, empty: emptyDoc, peerPassword, peerToken,
  };
})(window);
